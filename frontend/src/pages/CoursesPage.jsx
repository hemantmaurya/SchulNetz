...
function CoursesPage() {
  ...
  const fetchRecords = async (page = 1, newLimit = limit) => {
    try {
      const res = await API.get(`/api/courses?page=${page}&limit=${newLimit}`);
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      await API.put(`/api/courses/${currentRecord.id}`, formData);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await API.delete(`/api/courses/${currentRecord.id}`);
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-black">CoursesPage Management</h1>
        <button
